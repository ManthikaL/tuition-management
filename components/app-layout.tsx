        ...
        <div className="p-6 flex items-center justify-between">
          {sidebarOpen && (
            <Link href="/dashboard" className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg hidden md:inline">Tuition</span>
            </Link>
          )}
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-2 overflow-y-auto">
          {menuItems.map(item => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors group relative"
            >
